...
public class Board {
	...
	public List<Move> getPossibleMoves(Color c){
		...
		for(int i=0;i<moves.size(); i++){
			if(movePossible(moves.get(i)) == true){
				returnMoves.add(moves.get(i));
				//System.out.println("Move Accepted:" + moves.get(i).getMoveString() );
			}
		}	
	...
	public boolean movePossible(Move m){
		...
		if(m.getPosition1().getX() == 3 && m.getPosition1().getY() == 7 && moving.hasMoved == false){
			...
			if(m.getPosition2().getX() == 1 && m.getPosition2().getY() == 7 && castleInUse != null && castleInUse.hasMoved == false){
				...
				if(canCastle){
					castleInUse.getPosition().setPosition(2, 7);
					return true;
				}
				
				
			}
			castleInUse = findAtPosition(new Position(7,7));
			//// QUEEN SIDE BLACK
			if(m.getPosition2().getX() == 5 && m.getPosition2().getY() == 7 && castleInUse != null && castleInUse.hasMoved == false){
				...
				if(findAtPosition(new Position(4,7)) != null){
					canCastle = false;
				}
				if(canCastle){
					castleInUse.getPosition().setPosition(5, 7);
					return true;
				}
		...
		if(m.getPosition1().getX() == 3 && m.getPosition1().getY() == 0 && moving.hasMoved == false){
			...
			//// KING SIDE WHITE	
			castleInUse = findAtPosition(new Position(0,0));
			if(m.getPosition2().getX() == 1 && m.getPosition2().getY() == 0 && castleInUse != null && castleInUse.hasMoved == false ){
				System.out.println("TEst");
				if(findAtPosition(new Position(1,0)) != null){
					canCastle = false;
				}
				if(findAtPosition(new Position(2,0)) != null){
					canCastle = false;
				}
				...
				if(canCastle){
					castleInUse.getPosition().setPosition(2, 0);
					return true;
				}
				
			}
			//// QUEEN SIDE WHITE
			castleInUse = findAtPosition(new Position(7,0));
			if(m.getPosition2().getX() == 5 && m.getPosition2().getY() == 0 && castleInUse != null && castleInUse.hasMoved == false){
				...
				if(findAtPosition(new Position(4,0)) != null){
					canCastle = false;
				}
				
				if(canCastle){
					castleInUse.getPosition().setPosition(4, 0);
					return true;
				}
